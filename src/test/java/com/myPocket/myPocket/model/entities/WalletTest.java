...
public class WalletTest {
    ...
    @Test
    public void returnsRevenuesMap() {
        TestEnvironment testEnv = new TestEnvironment().createBasicEnvironment();
        ...
        firstAccount.addRevenue(new Revenue(testEnv.getWorkRevenueCategory(), firstAccount, 1000));
        firstAccount.addRevenue(new Revenue(testEnv.getWorkRevenueCategory(), firstAccount, 500));
        firstAccount.addRevenue(new Revenue(testEnv.getPassiveRevenueCategory(), firstAccount, 30));

        secondAccount.addRevenue(new Revenue(testEnv.getWorkRevenueCategory(), secondAccount, 5000));
        secondAccount.addRevenue(new Revenue(testEnv.getWorkRevenueCategory(), secondAccount, 150));
        secondAccount.addRevenue(new Revenue(testEnv.getPassiveRevenueCategory(), secondAccount, 21));

        Map<RevenueCategory, Double> valuesByRevenueCategory = testWallet.getRevenueMap();

        assertEquals(6650, valuesByRevenueCategory.get(testEnv.getWorkRevenueCategory()));
        assertEquals(51, valuesByRevenueCategory.get(testEnv.getWorkRevenueCategory()));
    }
